...
def build_order_response_for_request(session: Session, order: CommissionRequest, viewer_is_admin: bool) -> CommissionOrderResponse:
    ...
    payable_amount_cents = order.quote_amount_cents
...
def list_admin_orders(page: int = Query(default=1, ge=1), page_size: int = Query(default=10, ge=1, le=50), authorization: str | None = Header(default=None)) -> PaginatedOrdersResponse:
    with SessionLocal() as session:
        ...
        summaries = [
            ...
            CommissionOrderSummaryResponse(order_number=order.order_number, order_kind="gallery", customer_name=order.customer_name, customer_email=order.customer_email, category_name=order.item_title, status=order.status.name if order.status else STATUS_ACCEPTED, amount_cents=order.amount_cents, customer_confirmed_at=order.customer_confirmed_at, can_open=True, created_at=order.created_at, updated_at=order.updated_at)
            for order in gallery_orders
        ] + [
            CommissionOrderSummaryResponse(order_number=order.order_number, order_kind="gallery_inquiry", customer_name=order.customer_name, customer_email=order.customer_email, category_name=order.item_title, status=STATUS_SUBMITTED, amount_cents=order.amount_cents, customer_confirmed_at=None, can_open=True, created_at=order.created_at, updated_at=order.updated_at)
