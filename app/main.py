...
from app.models import Base, CommentAuthorRole, CommissionCategory, CommissionComment, CommissionFile, CommissionRequest, CommissionStatus, GalleryItem, User, UserRole
...
class CommissionFileResponse(BaseModel):
    id: int
    file_name: str
    file_url: str
    content_type: str
...
class CommissionCommentRequest(BaseModel):
    body: str
...
class CommissionOrderSummaryResponse(BaseModel):
    order_number: str
    customer_name: str
    category_name: str
    status: str
...
def build_comment_response(comment: CommissionComment, latest_customer_comment_id: int | None, latest_admin_comment_id: int | None) -> CommissionCommentResponse:
    ...
    return CommissionCommentResponse(
        id=comment.id,
        author_role=comment.author_role.value,
        body=comment.body,
        email_sent_at=comment.email_sent_at,
        created_at=comment.created_at,
        updated_at=comment.updated_at,
        can_send_email=comment.id == latest_id and comment.email_sent_at is None,
    )
...
def build_order_summary_response(order: CommissionRequest, category_by_id: dict[int, CommissionCategory]) -> CommissionOrderSummaryResponse:
    return CommissionOrderSummaryResponse(
        order_number=order.order_number,
        customer_name=order.customer_name,
        category_name=build_category_name(order, category_by_id),
        status=order.status.value,
        quote_amount_cents=order.quote_amount_cents,
...
def build_order_response(order: CommissionRequest, categories: list[CommissionCategory], files: list[CommissionFile], comments: list[CommissionComment], viewer_is_admin: bool) -> CommissionOrderResponse:
    ...
    return CommissionOrderResponse(
        ...
        custom_category_name=order.custom_category_name,
        ...
        comments=[build_comment_response(comment, latest_customer_comment_id, latest_admin_comment_id) for comment in comments],
    )
...
@app.get("/commission-categories", response_model=list[CategoryResponse])
...
@app.get("/admin/commission-categories", response_model=list[CategoryResponse])
...
@app.post("/admin/commission-categories", response_model=CategoryResponse)
...
@app.patch("/admin/commission-categories/{category_id}", response_model=CategoryResponse)
...
@app.get("/admin/orders", response_model=PaginatedOrdersResponse)
...
@app.post("/commissions", response_model=CommissionOrderResponse)
async def create_commission_request(
    customer_name: str = Form(...),
    customer_email: EmailStr = Form(...),
    customer_phone: str = Form(...),
    category_id: int | None = Form(default=None),
    custom_category_name: str = Form(default=""),
...
@app.patch("/orders/{order_number}/comments/{comment_id}", response_model=CommissionCommentResponse)
...
@app.delete("/orders/{order_number}/comments/{comment_id}")
...
@app.post("/orders/{order_number}/comments/{comment_id}/send-email", response_model=CommissionCommentResponse)
...
@app.post("/orders/{order_number}/decline", response_model=CommissionOrderResponse)
...
@app.post("/orders/{order_number}/confirm-payment", response_model=CommissionOrderResponse)
